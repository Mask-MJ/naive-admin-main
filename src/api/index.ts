...
import { ContentTypeEnum, RequestEnum, ResultEnum, TOKEN_KEY } from './request/httpEnum';
import { formatRequestDate, joinTimestamp } from './request/helper';
...
const transform: AxiosTransform = {
  ...
  responseInterceptorsCatch: (error: any) => {
    ...
    try {
      ...
      if (err?.includes('Network Error')) errMessage = t('api.networkExceptionMsg');
    ...
    const errInfo = checkStatus(error?.response?.status, msg);
    errInfo &&
      window.$message.error(`global_error_message_status_${error?.response?.status} ${errInfo}`);
    return Promise.reject(error);
  },
};
